...
/* ------------------------------------------------------------------ */
/* Layout griglia                                                       */
/* 4 colonne x 2 righe = 8 canali per pagina (come Wii)               */
/* ------------------------------------------------------------------ */
#define COLS_PER_PAGE   4
#define ROWS_PER_PAGE   2
#define SLOTS_PER_PAGE  (COLS_PER_PAGE * ROWS_PER_PAGE)
...
bool channel_grid_load(ChannelGrid *grid, SDL_Renderer *renderer,
    ...
    /* Carica texture per i canali della pagina corrente */
    for (int i = 0; i < grid->count; i++) {
...
/* Restituisce l'indice del canale a page/slot dato */
...
static void switch_page(ChannelGrid *grid, int new_page,
                        SDL_Renderer *renderer) {
    if (new_page < 0 || new_page >= grid->total_pages) return;

    /* Scarica texture pagina corrente per risparmiare RAM */
    for (int i = 0; i < grid->count; i++) {
        if (grid->channels[i].page == grid->current_page) {
            channel_free_textures(&grid->channels[i]);
        }
    ...
    /* Carica texture nuova pagina */
    for (int i = 0; i < grid->count; i++) {
        if (grid->channels[i].page == new_page) {
            channel_load_textures(&grid->channels[i], NULL); /* renderer non disponibile qui */
        }
    }

    /* Seleziona slot 0 della nuova pagina */
...
/* ------------------------------------------------------------------ */
/* Animazioni                                                           */
/* Aggiornamento leggero basato su dt (secondi)                        */
/* ------------------------------------------------------------------ */
...
void channel_render(SDL_Renderer *renderer, const Channel *ch,
    ...
    /* Calcola rect con scala applicata */
    ...
    /* Label sotto l'icona */
    /* Il testo viene renderizzato dal renderer principale
       che ha accesso al font — qui disegniamo solo la geometria */
}
