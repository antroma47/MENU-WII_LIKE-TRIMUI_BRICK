...
static void theme_set_defaults(void) {
    ...
    g_theme.colors.banner_bg        = (SDL_Color){220, 240, 255, 240};
...
static void theme_parse_conf(const char *path) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        else if (!strcmp(key, "banner_bg"))        parse_color(val, &c->banner_bg);
...
static const int FONT_SIZES[FONT_COUNT] = {10, 14, 20};
